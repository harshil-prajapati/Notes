...
|----------------------------|-----------------------------------|----------------------------------|---------------------------------|---------------------------------|
| **Definition**              | A search algorithm that checks each element in a list one by one. | A search algorithm that finds the position of a target value within a sorted array by repeatedly dividing the search interval in half. | A linear data structure that follows the Last In First Out (LIFO) principle. | A linear data structure that follows the First In First Out (FIFO) principle. |
| **Search Method**           | Linear search (checks each element one by one). | Divide and conquer (splits the list into two halves). | N/A                             | N/A                             |
| **Time Complexity**         | O(n)                              | O(log n)                         | O(1) for push and pop            | O(1) for enqueue and dequeue    |
| **Space Complexity**        | O(1)                              | O(1)                              | O(n)                             | O(n)                            |
| **Best Case**               | O(1) (If the element is the first element). | O(1) (If the target is the middle element). | O(1)                             | O(1)                             |
